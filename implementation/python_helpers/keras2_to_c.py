...
class Layer(object):
    def __init__(self,prevlayers,layers,layer_count):
        self.name = '%s_%02d' % (layers[0][k_name],layer_count)
    ...
    def get_activation(self):
        ...
        names = [layer[k_name] for layer in self.layers]
        activation = zero_layer['config']['activation']

        if 'Activation' in names:
            idx = names.index('Activation')
            if self.layers[idx]['config'].has_key('activation'):
                activation = self.layers[idx]['config']['activation']
...
class ConvLayer(Layer):
    ...
    def write(self,input_shape,f):
        ...
        f.write('const static ConvLayer2D_t %s = {&%s,&%s,%s,%s,%s,TOFIX(%f),%s};\n' % (self.name.lower(),weights_name,bias_name,output_name,input_name,poolvar,0,k_activation_func_map[self.get_activation()]))
        f.write('\n\n\n')
...
class Dense(Layer):
    def write(self,input_shape,f):
        ...
        f.write('const static FullyConnectedLayer_t %s = {&%s,&%s,%s,%s,TOFIX(%f),%s,%d};\n' % (self.name.lower(),weights_name,bias_name,output_name,input_name,0,activation_function,use_softmax))
        f.write('\n\n\n')
        return output_shape
...
class Lstm(Layer):
    def write(self,input_shape,f):
        ...
        #in Keras 2.x, there are three weight matrices
        # input weights -- Ninputs x 4*Nunits
        # cell weights -- Ncell x 4*Nunits
        # biases --  4 * Nunits
        #
        # Our output format for the matrices
        # is the four separate gates [input,forget,cell,output]
        # and the dims are  Nunits x (Ninput + Ncell)
        #   
        ...
        f.write('const static LstmLayer_t %s = {%s,%s,%s,TOFIX(%f),%s};\n\n\n' % (objname,gates_names,output_name,input_name,0,activation_function))
        return output_shape
...
def write_sequential_network(layerobjs,model,f):
    ...
    if 'batch_input_shape' in first_layer.keys():
        input_shape = (1,1,1,first_layer['batch_input_shape'][2])
    else:
        layer_input_shape = first_layer['input_shape']
...
def save_model_to_c(model,name):
    ...
    first = True
    for layer in config:
        if layer[k_name] in layer_map.keys():
            if not first:
