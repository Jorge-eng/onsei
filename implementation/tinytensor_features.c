...
#define PREEMPHASIS (31129) //0.95 in Q15
...
static void get_speech_energy_ratio(int16_t * fr,int16_t * fi,int16_t input_scaling) {
    ...
    //log (a/b) = log(a) - log(b)
    temp32 = FixedPointLog2Q10(speech_energy) - 2*input_scaling * 1024;// - FixedPointLog2Q10(total_energy);
...
void tinytensor_features_add_samples(const int16_t * samples, const uint32_t num_samples) {
    ...
    if (add_samples_and_get_mel(&maxmel,&avgmel,melbank,samples,num_samples)) {
        ...
        if (_this.results_callback) {
            _this.results_callback(_this.results_context,melbank8,feats_flags);
        }
