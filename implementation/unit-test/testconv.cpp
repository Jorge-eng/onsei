...
#define NO_DROPOUT (0)
const static ConvLayer2D_t conv_layer_def2m = {&weights2m,&biases2,ref2_dims,image2m_dims,NO_DROPOUT,tinytensor_linear};
...
TEST_F(TestConv,TestSmallConvLayerMultipleImageSecondImageNone) {
    
    tensor_in = tinytensor_clone_new_tensor(&image2m2);
    
    ConstLayer_t layer = tinytensor_create_conv_layer(&conv_layer_def2m);
...
TEST_F(TestConv,TestSmallConvLayerMultipleImageFirstImageNone) {
    
    tensor_in = tinytensor_clone_new_tensor(&image2m);
...
TEST_F(TestConv, TestVerySimpleTwoImageInput) {
    ...
    ConvLayer2D_t conv_context = {&conv_x,&conv_bias,output_dims,input_dims,0,tinytensor_linear};
    ...
    ASSERT_NEAR(tensor_out->x[0],accumulator,5);
    ...
    ASSERT_NEAR(tensor_out->x[1],accumulator,5);
    ...
    ASSERT_NEAR(tensor_out->x[2],accumulator,5);
    ...
    ASSERT_NEAR(tensor_out->x[3],accumulator,5);
...
TEST_F(TestConv,TestSmallConvNetwork) {
    tensor_in = tinytensor_clone_new_tensor(&testinput_smallnet);
    
    ConstSequentialNetwork_t net = initialize_network();
    
    tensor_out = eval_net(&net,tensor_in);
    
    const uint32_t len = testoutput_smallnet_dims[0]*testoutput_smallnet_dims[1] * testoutput_smallnet_dims[2] * testoutput_smallnet_dims[3];
    
    for (int i = 0; i < 4; i++) {
        ASSERT_EQ(testoutput_smallnet_dims[i],tensor_out->dims[i]);
    }
    
    for (int i = 0; i < len; i++) {
        int val1 = testoutput_smallnet.x[i];
        if (val1 > 127) {
            val1 = 127;
        }
        ...
        ASSERT_NEAR(val1,tensor_out->x[i],5);
    }
