...
TEST_F(TestNet, test_input_rand) {
    ...
    tensor_out = tinytensor_eval_net(&net,tensor_in,NET_FLAGS_NONE);
...
TEST_F(TestNet, kwClip_160517_02_1_layer1) {
    ...
    ConstSequentialNetwork_t net = initialize_network();
    tensor_out = tinytensor_eval_partial_net(&net,tensor_in,1,NET_FLAGS_NONE);
    uint32_t len = 1;
    for (int i = 0; i < 4; i++) {
...
TEST_F(TestNet, kwClip_160517_02_1_layer2) {
    ...
    ConstSequentialNetwork_t net = initialize_network();
    tensor_out = tinytensor_eval_partial_net(&net,tensor_in,2,NET_FLAGS_NONE);
    uint32_t len = 1;
    for (int i = 0; i < 4; i++) {
