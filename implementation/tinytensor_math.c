...
int32_t tinymath_abs_int16(int16_t x) {
    if (((uint32_t)x) == 0x8000) {
        x++;
    }
        ...
        for (ipool_col = 0; ipool_col < num_pool_cols; ipool_col++)
        {
            ...
            for (ioutrow = outrow_start; ioutrow < outrow_end; ioutrow++)
            {
                for (ioutcol = outcol_start; ioutcol < outcol_end; ioutcol++)
                {
                    ...
                    const ImagePixel_t * image_start = image +  (ioutrow * num_image_cols) + ioutcol;
                    ...
                    for (iimage = 0; iimage < num_images; iimage++) {
                        ...
                        const ImagePixel_t * image_row = image_start;
            ...
            //accumulator is in Q8 + Q16 = 32Q24 format here
            //now do dropout
            temp64 = temp32 * dropout_weight;
            temp64 >>= QFIXEDPOINT;
            
            //if bias scaling is greater than weight + input scaling
            //then left shift it so it matches
            bias_scaling_diff = weight_scaling + input_scaling - bias_scaling;
            
            bias32 = bias; //Q16
            bias32 <<= QFIXEDPOINT; //Q24
            ...
            activation(&temp_pixel,&temp_scale,(int32_t)temp64,input_scaling);
            assert(temp_scale == 0);
            out_row[ipool_col] = temp_pixel;
        }
        //printf("-----\n");
