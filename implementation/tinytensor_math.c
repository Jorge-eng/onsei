...
void tinytensor_tanh(Weight_t * y, int8_t * out_scale, int32_t x,int8_t in_scale) {
    const static uint32_t k_max_len = sizeof(tanh_table_q7) / sizeof(tanh_table_q7[0]);
...
void tinytensor_vec_softmax_in_place(Weight_t * xvec, uint32_t len, int8_t in_scale) {
    ...
    int32_t expval;
    temp32 = 0;
    const static int8_t shift = QFIXEDPOINT - 10;
    for (i = 0; i < len; i++) {
        val = xvec[i];
        ...
        if (shift >= 0) {
            expval = FixedPointExp2Q10(val >> shift);
        }
        else {
            expval = FixedPointExp2Q10(val << -shift);
        }
